...
AddEventHandler("MMA:createFighter", function(data)
    for key, value in pairs(data) do
        local pedData = data[key]
        local model = pedData.model
        local hash = GetHashKey(model)

        local fighter = CreatePed(5, hash, -293.4473, -1992.72, 30.96, 258.80, false, true)
        --NetworkRegisterEntityAsNetworked(fighter)
        --SetNetworkIdCanMigrate(NetworkGetNetworkIdFromEntity(fighter), true)
        --SetNetworkIdExistsOnAllMachines(NetworkGetNetworkIdFromEntity(fighter), true)
        Wait(500)
        --SetPedFleeAttributes(fighter, 0, false)
        --SetPedCombatAttributes(fighter, 46, 1)
        --SetPedCombatAbility(fighter, 100)
        --SetPedCombatMovement(fighter, 2)
        ...
        --SetEntityAsMissionEntity(fighter)
